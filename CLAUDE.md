...
4. **`index/module_resolver.rs` calls the builder, then queries `FileAnalysis`** — never walks the tree directly.
...
- `lsp/backend/` — `LanguageServer` impl, request routing (parts: `server` trait impl, `indexing`, `lifecycle`, `completion`).
- `index/document.rs` — open-file `Document` (tree + text + analysis + stable_outline).
- `index/file_store.rs` — unified store for open + workspace FileAnalyses, role-tagged, dedup'd by path.
- `model/file_analysis/` — data model; serde-derived (parts: `core_types`, `types`, `dispatch`, `cross_file`, `imports`, `outline`, `lifecycle`, `queries`, `enrichment`, `class_queries`, `cursor_queries`, `resolution`, `completion`).
- `build/builder/` — CST→FileAnalysis. ONLY tree-sitter consumer (parts: `pipeline`, `visit_decl`/`visit_use`/`visit_calls`/`visit_method`, `emit`, `chain`, `fold`, `frameworks`, `arity`, `infra`, `helpers`, `plugin_emit`, `narrowing`, `pattern_dispatch`).
- `build/language_driver.rs` — `LanguageDriver`/`PackDriver`/`LanguageRegistry`, the multi-language serving seam (`docs/prompt-multi-language.md`). `PackDriver::analyze_with_path`'s pipeline is 8 named phases (doc comment on that fn): gather context → transform+parse → extract → remap spans → enrich skeleton → `into_file_analysis` → `emit_return_fuel` → register post-build hooks; remap must precede injection, injection must precede `into_file_analysis`. `emit_return_fuel` is the post-assembly auto-return / implicit-`this` pass (cpp semantics): auto-return chain-fuel always, plus — gated by the pack's `implicit_this_members` capability — implicit-`this->field` read edges AND bare sibling-method-call pinning (`resolved_package` → enclosing class).
...
- `build/pod.rs` — POD→markdown via tree-sitter-pod.
- `lsp/cursor_context.rs` — position-dependent context (Perl completion/sig-help).
...
- `index/module_index/` — cross-file dep API, `CachedModule`, `SubInfo`, reverse index (`func → modules`).
- `index/module_resolver.rs` — background `std::thread`, in-process parsing, workspace indexing (Rayon).
...
`enrich_imported_types_with_keys()` on `FileAnalysis` propagates imported return types and hash keys: pushes `TypeConstraint`s for call bindings to imported funcs, injects synthetic `HashKeyDef` symbols for cross-file hash-key completion. Idempotent: `base_type_constraint_count` / `base_symbol_count` / `base_witness_count` set after initial build; enrichment truncates back to baseline before appending. `rebuild_enrichment_indices()` rebuilds index maps after. Called IN-PLACE from `publish_diagnostics()` and the resolver refresh callback — i.e. open documents. Closed workspace/dependency files answer enriched through **the R4 overlay** (`ModuleIndex::enriched_snapshot` / `CrossFileLookup::enriched_present`): derived fingerprint+generation-keyed copies, byte-capped, cycle-guarded, consulted FALLBACK-ON-MISS by `query_sub_return_type`'s imported recursion and the `MethodOnClass` cross-file primary (and by `--check`/`--dump-package`). Never hand-roll a deep-copy+enrich dance — route through the overlay. Type/method resolution through inheritance survives this via the `MethodOnClass` query-time walk; **dispatch-verb resolution** survives it via the query-time `ReceiverGated` seam (`FileAnalysis::applicable_dispatches`, resolved in `refs_to` / goto-def — `docs/adr/receiver-gated-dispatch.md`). Still deferred: `ClassIsa`/`param_types` ancestry-gated *emission* (Phase 2 mints these on the same seam) — see `docs/prompt-enrichment-inheritance-residual.md` for the per-manifest applicability matrix.
