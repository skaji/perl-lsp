...
- `module_cache.rs` — SQLite (schema v9): bincode+zstd FileAnalysis blobs PLUS the relational ref index (`files`/`strings`/`refs` tables, `docs/adr/relational-ref-index.md`) — rows shredded from each analysis in the same txn as its blob, `REF_ROWS_VERSION` governs row format (wipe+re-shred, never a blob drop). `refs_to` retrieval is `ref_candidate_files` (indexed by `name_match_key`); the matcher runs on the rehydrated analysis (`refs_present`, the `bag_present` twin). Resident index copies are refs+bag-evicted after persist (`PERL_LSP_NO_EVICT` keeps both; `PERL_LSP_REF_ROWS=0` forces the resident-only walk — pair them). `--refs-parity <root> [--sample=N]` is the A/B migration net.
