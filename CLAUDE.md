...
- `parenthesized_expression` is **not a Perl grammar kind** — 0 in ts-parser-perl's `node-types.json`, so a `kind() == "parenthesized_expression"` arm in Perl-side code never matches (27 such dead comparisons at time of writing; `PARKED.md` tracks the sweep). Paren lists are `list_expression`; single-expression parens are invisible. It IS a real **tree-sitter-cpp** kind, so pack-side comparisons are live — don't delete those.
