...
- `model/file_analysis/` — data model; serde-derived (parts: `core_types`, `ref_table`, `types`, `dispatch`, `ancestry`, `cross_file`, `imports`, `outline`, `lifecycle`, `surface_feed`, `queries`, `enrichment`, `class_queries`, `cursor_queries`, `invocants`, `hover`, `sym_index`, `completion`). `RefTable` (`ref_table`) is the reference axis: the refs, every index over them, their `evict()`, their heap arm and their enrichment baseline in one owner — `FileAnalysis::refs()` reads it, the ref-shaped queries delegate to it.
...
8. `FileAnalysis::new(...)` — construct FA, build indices; finalize runs the MCB→bag bridge (`emit_method_call_binding_edges`: each `$var = $invocant->method()` binding → `Variable → Edge(MethodOnClass)` witness, chased lazily by the registry with whatever index the query holds; enrichment re-runs the same bridge).
