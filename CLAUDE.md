...
- `cursor_slot.rs` — the cursor `Slot` taxonomy (`docs/adr/cursor-slots.md`): one vocabulary ("what kind of hole is the cursor in") over both `cursor_context` (Perl) and `cursor_sentinel` (pack). `detect_slot` answers identity (Member/Key/Identifier/Import/ModulePath); `detect_call_slot` answers arg-position orthogonally. Consumers switch on `Slot`, never on language.
