...
use crate::model::file_analysis::{CrossFileLookup, FileAnalysis, InferredType, Span};
...
module_index: Option<&dyn CrossFileLookup>,
    ...
    match node.kind() {
        ...
        "method_call_expression" => {
            ...
            if crate::model::conventions::is_constructor_name(method_text) {
                    ...
                    if invocant_node.end_position() < point {
                        ...
                        if let Some(method) = current.child_by_field_name("method") {
                            if let Ok(mtext) = method.utf8_text(source) {
                                if let crate::model::conventions::MethodToken::Qualified {
                                    package, ..
                                } = crate::model::conventions::MethodToken::parse(mtext)
                                {
...
pub fn build_plugin_query_context(
    ...
    source: &[u8],
    point: Point,
) -> Option<crate::build::plugin::SigHelpQueryContext> {
    use crate::build::plugin::{CallFrame, ContainerFrame, ContainerKind, SigHelpQueryContext};
    ...
    let call = call_ctx.as_ref().map(|c| {
        ...
        let args = match c.first_arg_string.as_ref() {
            Some(s) => vec![crate::build::plugin::ArgInfo {
                text: s.clone(),
                ...
                value_shape: crate::build::plugin::ValueShape::Str(s.clone()),
                sub_params: Vec::new(),
