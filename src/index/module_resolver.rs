...
pub fn pack_file_changed(
    ...
    let mut changed_fa: Option<Arc<crate::model::file_analysis::FileAnalysis>> = None;
    if !deleted {
        crate::build::cpp_reparse::evict_analysis_caches(&std::iter::once(canon.clone()).collect());
    ...
    let mut results: Vec<(PathBuf, Arc<crate::model::file_analysis::FileAnalysis>)> = targets
        .par_iter()
        .filter_map(|p| {
            let reg = crate::build::language_driver::LanguageRegistry::with_enabled();
