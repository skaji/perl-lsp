...
fn bodyless_define_joins_the_config_universe() {
    use crate::build::cpp_macro_model::{classify, KnownConfig, Reachability};
    let mut p = cpp_parser();
    let src = "\
#define MY_FEATURE
#ifdef MY_FEATURE
...
";
    ...
    let defs = crate::build::cpp_reparse::collect_macro_defs(&mut p, src);
...
";
    ...
    let refs = crate::build::cpp_reparse::macro_body_name_refs(&mut p, src, &known);
...
";
    let defs = crate::build::cpp_reparse::collect_macro_defs(&mut p, src);
    let by_name: std::collections::HashMap<&str, &crate::model::file_analysis::MacroDef> =
...
fn classify_param_return_reads_param_index() {
    ...
    // Bare identity.
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "(x)", &params(&["x"])),
        Some(0)
    );
    // Cast wrapper — the cast type is ignored; still the argument's value.
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "((Widget*)(x))", &params(&["x"])),
        Some(0)
    );
    // Two-param select-second.
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "(b)", &params(&["a", "b"])),
        Some(1)
    );
    // An operator body is param-INDEPENDENT, not an identity.
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "((x)*(x))", &params(&["x"])),
        None
    );
    // A bare identifier that isn't a parameter (a global) is not a param return.
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "(GLOBAL)", &params(&["x"])),
        None
    );
    ...
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "(void)(x)", &params(&["x"])),
        Some(0)
    );
    assert_eq!(
        crate::build::cpp_reparse::classify_param_return(&mut p, "((void) v)", &params(&["v"])),
        Some(0)
    );
}
...
struct sv { REFCNT };
";
    let plan = crate::build::cpp_reparse::plan_member_blocks(&mut p, src);
...
fn include_closure_reports_incomplete_on_unreadable_header() {
    ...
    let (closure, complete) = crate::build::cpp_reparse::include_closure(&main_c, complete_src);
    assert!(complete, "a fully-readable closure is complete");
    assert_eq!(closure.len(), 1);

    let trunc_src = "#include \"bad.h\"\n";
    std::fs::write(&main_c, trunc_src).unwrap();
    let (_closure, complete) = crate::build::cpp_reparse::include_closure(&main_c, trunc_src);
    ...
    let (closure, complete) = crate::build::cpp_reparse::include_closure(&main_c, unresolved_src);
...
fn predefined_macros_seed_reachability_config() {
    ...
    let mut defined = std::collections::HashSet::new();
    let mut universe = std::collections::HashSet::new();
    crate::build::cpp_reparse::seed_predefined(
        &mut defined,
        &mut universe,
...
fn context_free_safe_macro_survives_conditional_region_drop() {
    ...
    let tree = parse(&mut p, src);
    let before = crate::build::cpp_reparse::parse_damage(tree.root_node());
    let (wide, _) = preprocess_with(&tree, src, &external);
    let wide_dmg = crate::build::cpp_reparse::parse_damage(parse(&mut p, &wide).root_node());
    ...
    assert!(
        crate::build::cpp_reparse::parse_damage(parse(&mut p, &rw).root_node()) <= before,
...
fn salvage_keeps_context_free_safe_groups_without_a_probe() {
    ...
    let base = (
        crate::build::cpp_reparse::parse_damage(base_tree.root_node()),
...
int real_symbol;\n\
#endif\n";
    let guards = crate::build::cpp_reparse::collect_include_guard_names(&mut p, src);
